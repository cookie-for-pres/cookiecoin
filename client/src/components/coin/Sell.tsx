...
const Sell = ({ coin: c, boughtCoin: bc, coinId, cookie, balances: b }: any) => {
  ...
  useEffect(() => {
    ...
    const amount: number = parseFloat(boughtCoin.amount);
  ...
  const sell = async () => {
    const req = await fetch('http://127.0.0.1:5500/api/coins/sell', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
